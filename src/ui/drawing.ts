...
export type EditKind = 'replace' | 'branch' | 'continue' | 'loose' | 'variation' | 'swap' | 'fill' | 'remove'
...
export function branchChoice(options: readonly BranchOption[], points: Point[]): EditChoice {
  return {
    ...
    options: options.map((option) => ({
      ...
      kind: option.kind === 'end' ? ('continue' as const) : option.kind === 'loose' ? ('loose' as const) : ('branch' as const),
...
export function describeBranchOption(option: BranchOption): string {
  ...
  if (option.kind === 'loose') return t('branch.option.loose')
